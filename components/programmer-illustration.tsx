...
// Keyboard key rects: [x, y, width] in parent-group coordinate space
const KB: [number, number, number][] = [
  [557.798, 845.887, 6.833],  [595.05,  845.887, 6.833],  [605.604, 845.887, 6.833],
  [567.931, 845.588, 23.819], [617.599, 845.588, 23.819],
  [567.732, 853.957, 6.833],  [604.983, 853.957, 6.833],  [615.538, 853.957, 6.833],
  [577.865, 853.659, 23.819], [627.532, 853.659, 23.819],
  [577.666, 862.029, 6.833],  [614.916, 862.029, 6.833],  [625.471, 862.029, 6.833],
  [587.798, 861.73,  23.819], [637.466, 861.73,  23.819],
  [587.599, 870.099, 6.833],  [624.85,  870.099, 6.833],  [635.404, 870.099, 6.833],
  [597.732, 869.801, 23.819],
  [595.05,  902.193, 6.833],  [605.604, 902.193, 6.833],
  [617.599, 902.491, 23.819], [557.798, 902.193, 6.833],  [567.931, 902.491, 23.819],
  [567.732, 894.122, 6.833],  [604.983, 894.122, 6.833],  [615.538, 894.122, 6.833],
  [577.865, 894.42,  23.819],
  [577.666, 886.051, 6.833],  [614.916, 886.051, 6.833],  [625.471, 886.051, 6.833],
  [587.798, 886.349, 23.819],
  [587.599, 877.98,  6.833],  [624.85,  877.98,  6.833],  [635.404, 877.98,  6.833],
  [597.732, 878.278, 23.819],
]
const KB_H = 61 // scroll block height in SVG units (one full keyboard grid height)

// Monitor code lines: [indent, width, color]
const MON: [number, number, string][] = [
  [0,  120, '#4a6fa5'],
  [0,    0, ''],
  [10, 180, '#3f3d56'], [10, 110, '#3f3d56'],
  [20, 150, '#6b7fde'], [20,  90, '#6b7fde'], [20, 130, '#6b7fde'],
  [10,  20, '#3f3d56'],
  [0,    0, ''],
  [0,  140, '#4a6fa5'],
  [10, 200, '#3f3d56'], [10, 140, '#3f3d56'],
]

export function ProgrammerIllustration({ className }: { className?: string }) {
  return (
    <div className={className ?? ""}>
      <style>{`
        @keyframes pg-glow {
          0%, 100% { opacity: 1;    filter: brightness(1)    drop-shadow(0 0 0px  rgba(0,155,222,0));   }
          25%       { opacity: 0.87; filter: brightness(1.25) drop-shadow(0 0 10px rgba(0,155,222,0.9)); }
          55%       { opacity: 0.93; filter: brightness(0.9)  drop-shadow(0 0 5px  rgba(0,155,222,0.5)); }
          80%       { opacity: 1;    filter: brightness(1.15) drop-shadow(0 0 8px  rgba(0,155,222,0.7)); }
        }
        .pg-screen { animation: pg-glow 2.8s ease-in-out infinite; }

        @keyframes pg-type-loop {
        ...
        .pg-mon rect {
          ...
          animation: pg-type-loop 7s ease-in-out infinite both;
        }
        @keyframes pg-blink {
          0%, 100% { opacity: 1; }
          50%       { opacity: 0; }
        }
        .pg-cursor { animation: pg-blink 1s step-end infinite; }
      `}</style>
      ...
      <svg
        ...
        role="img"
        aria-label="Developer actively programming at a computer"
        style={{ width: "100%", height: "auto", display: "block" }}
      >
        <defs>
          {/* laptop (blue) screen bounds — clips the keyboard key scroll */}
          <clipPath id="pg-laptop-clip">
            <rect x={540.595} y={827.469} width={143.04} height={95.73} rx={2.521} />
          </clipPath>
        </defs>
        ...
        <g transform="translate(-28.583 -435)">
          ...
          {/* Central laptop screen (blue) — glow animation */}
          ...
          {/* Laptop keyboard keys — scrolling inside the screen clip */}
          <g clipPath="url(#pg-laptop-clip)">
            <g>
              <animateTransform
                ...
                calcMode="linear"
              />
              {/* block 1 — original positions */}
              {KB.map(([x, y, w], i) => (
                <rect key={i} x={x} y={y} width={w} height={3.576} rx={1.561} fill="#fff" />
              ))}
              {/* block 2 — duplicated KB_H units below for seamless loop */}
              {KB.map(([x, y, w], i) => (
                <rect key={`k2-${i}`} x={x} y={y + KB_H} width={w} height={3.576} rx={1.561} fill="#fff" />
              ))}
            </g>
          </g>
          ...
          {/* Large monitor white screen — glow effect */}
          <g className="pg-screen">
            <path
          ...
          {/* Monitor code lines — typewriter loop + blinking cursor */}
          <svg x={64.762} y={647.227} width={323.555} height={214.676} overflow="hidden" className="pg-mon">
            {(() => {
              let idx = 0
              return MON.map(([indent, w, color], i) =>
                w > 0 ? (
                  <rect
                    key={i}
                    x={12 + indent}
            ...
            <rect
              x={12} y={182} width={8} height={10} rx={1}
              fill="#009bde"
              className="pg-cursor"
            />
          </svg>
          ...
          {/* Three control pill buttons — render on top of scroll content */}
          <path
          ...
          {/* Character group — no arm animations */}
          <g transform="translate(250.837 684.649)">
            ...
            {/* Face */}
            <ellipse
              cx="51.737" cy="51.737" rx="51.737" ry="51.737"
          ...
          {/* Decorative code-bar pills removed — replaced by typewriter animation */}
          <g transform="translate(152.761 713.325)" style={{ display: 'none' }}>
            <path
